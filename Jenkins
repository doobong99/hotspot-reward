    ...
    def ZCP_USERID='09798'
    def DOCKER_IMAGE='sample/gs-spring-boot-docker'
    def K8S_NAMESPACE='hotspot-reward'

    podTemplate(label:label,
        serviceAccount: "zcp-system-sa-${ZCP_USERID}",
        containers: [
        	containerTemplate(name: 'jnlp', image: 'hotspot-reward-registry.cloudzcp.io/cloudz/jnlp-slave:alpine', args:'${computer.jnlpmac} ${computer.name}'),
            containerTemplate(name: 'maven', image: 'hotspot-reward-registry.cloudzcp.io/cloudz/maven:3.5.2-jdk-8-alpine', ttyEnabled: true, command: 'cat'),
            // containerTemplate(name: 'docker', image: 'hotspot-reward-registry.cloudzcp.io/cloudz/docker:stable', ttyEnabled: true, command: 'cat'),
            containerTemplate(name: 'docker', image: 'docker:17-dind', ttyEnabled: true, command: 'dockerd-entrypoint.sh', privileged: true),
            // containerTemplate(name: 'kubectl', image: 'hotspot-reward-registry.cloudzcp.io/cloudz/k8s-kubectl:v1.9.9', ttyEnabled: true, command: 'cat')
